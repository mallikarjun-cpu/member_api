...
def add_member():
   ...
   db = get_db()
   db.execute('insert into mem_details(name, email, level) values (?, ?, ?)', [name, email, level])
   db.commit()
   
   member_cur = db.execute('select id, name, email, level from mem_details where name = ?', [name])
...
def get_members():
   db = get_db()
   members_cur = db.execute('select id, name, email, level from mem_details')
...
def get_member(member_id):
    db = get_db()
    member_cur = db.execute('select id, name, email, level from mem_details where id = ?', [member_id])
...
def edit_member(member_id):
   ...
   db = get_db()
   db.execute('update mem_details set name =?, email = ?, level = ? where id= ?', [name, email, level, member_id])
   db.commit()
...
def delete_member(member_id):
   db = get_db()
   db.execute('delete from mem_details where id = ?', [member_id])
   db.commit()
